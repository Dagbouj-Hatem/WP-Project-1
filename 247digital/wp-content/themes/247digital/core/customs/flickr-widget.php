<?php
add_action( 'widgets_init','WL_flicker_widget'); 
   function WL_flicker_widget() { return   register_widget( 'WL_footer_flicker_widget' ); }
/**
 * Adds widget for recent Post in footer.
 */
class WL_footer_flicker_widget extends WP_Widget { 
	...
	function __construct() {
		parent::__construct(
			'WL_footer_flicker_widget', // Base ID
			__('Enigma Flicker widget', 'weblizar'), // Name
			array( 'description' => __( 'Displays your latest Flicker photos.', 'weblizar' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if($number_of_image=='') { $number_of_image = 6; }
		if($flicker_id=='') { $flicker_id ='121500546@N06'; }
	...
	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] )  && isset( $instance[ 'number_of_image' ] ) && isset( $instance[ 'flicker_id' ] )) {
			...
			$title = __( 'Fliker PhotoStream', 'weblizar' );
			$number_of_image = 6;
			$flicker_id= "121500546@N06";
		}
		?>
		<p>
		...
		<p>
		<label for="<?php echo $this->get_field_id('number_of_image' ); ?>"><?php _e( 'Number of images to display:','weblizar' ); ?></label> 
